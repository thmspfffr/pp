...
for isubj = 1  : size(pooled_f22,4)
  ...
  m_subj(isubj) = mean(mean(mean(pooled_f22(:,:,:,isubj),1),2),3);
  isubj
  rand_idx = randperm(20);
  for iff = 1  : 25
    for i = 1 : 246
      
      % POLYFIT WITH DEG 2 and 1
      dat = zscore(squeeze(pooled_f22(iff,i,:,isubj))');  
      if any(dat==0)
        warning('!!!')
      end
...
% 
[~,p]=ttest(ppp(ifoi,:,:,:),zeros(size(ppp(ifoi,:,:,:))),'dim',4);
h = p<fdr1(p(:),0.1,0);
masked = 1;
...
print(gcf,'-dpdf',sprintf('~/pupmod/plots/pp_quad_masked%d_spatialmap_f%d_v%d.tiff',masked,ifoi,1))
...
%%
poly_par22(1,:) = polyfit(1:20,squeeze(mean(mean(m22(1,:,:,:),2),3)),2);
...
plot(1:20,poly_par22(1,1).*(1:20).^2 + poly_par22(1,2).*(1:20) + poly_par22(1,3))
...
plot(1:20,poly_par22(2,1).*(1:20).^2 + poly_par22(2,2).*(1:20) + poly_par22(2,3))
tp_editplots; xlabel('Pupil bin'); ylabel('Power [norm.]')
xlim([-1 21])
ylim([-0.1 0.2])
...
plot(1:20,poly_par22(3,1).*(1:20).^2 + poly_par22(3,2).*(1:20) + poly_par22(3,3))
tp_editplots; xlabel('Pupil bin'); ylabel('Power [norm.]')
xlim([-1 21])
ylim([-0.2 0.4])
...
plot(1:20,poly_par11(1,1).*(1:20).^2 + poly_par11(1,2).*(1:20) + poly_par11(1,3))
tp_editplots; xlabel('Pupil bin'); ylabel('Power [norm.]')
xlim([-1 21])
ylim([-0.2 0.4])
...
plot(1:20,poly_par11(2,1).*(1:20).^2 + poly_par11(2,2).*(1:20) + poly_par11(2,3))
tp_editplots; xlabel('Pupil bin'); ylabel('Power [norm.]')
xlim([-1 21])
ylim([-0.2 0.2])
...
plot(1:20,poly_par11(3,1).*(1:20).^2 + poly_par11(3,2).*(1:20) + poly_par11(3,3))
tp_editplots; xlabel('Pupil bin'); ylabel('Power [norm.]')
xlim([-1 21])
ylim([-0.2 0.4])

print(gcf,'-dpdf',sprintf('~/pp/plots/pp_invU_avg.pdf',v))