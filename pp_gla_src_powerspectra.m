...
%             pxx(:,:,iseg) = nan(size(fxx,1),size(tp_filt,2));
        ...
        if v<222
          [pxx_8799(:,:,iseg) ,fxx]=pwelch(seg_dat,hanning(opt.n_win),0,ff,400,'power');
        else
          [pxx_8799(:,:,iseg) ]=pwelch(seg_dat,hanning(800),0.5,ff,400,'power');  
        end
        
        for igrid = 1 : max(BNA.tissue_5mm(:))
          pxx_246(:,igrid,iseg) = mean(pxx_8799(:,BNA.tissue_5mm == igrid),2);
        end
