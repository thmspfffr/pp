...
%       
      ...
      pxx_all(:,:,:,subj_counter,iblock)= nan(100,41,39);
      pup_all(:,subj_counter,iblock)=nan(41,1);
      rrr(:,subj_counter,iblock) = nan(101,1);
    end
  end
end
...
colormap(redblue); axis([1 31 1 67])

subplot(3,3,4)
imagesc(squeeze(nanmean(nanmean(pxx_all(1:67,12,:,:),2),4))',[-5 5]); set(gca,'ydir','normal')
set(gca,'xtick',1:5:67,'xticklabel',fxx(2:5:end))
colormap(redblue);
tp_editplots

[h,p]=ttest(squeeze(pxx_all(1:67,12,:,:)),zeros(size(squeeze(pxx_all(1:67,12,:,:)))),'dim',3); h=p<fdr1(p(:),0.05,1);

subplot(3,3,5)
imagesc(squeeze(nanmean(nanmean(pxx_all(1:67,12,:,:),2),4))'.*h',[-5 5]); set(gca,'ydir','normal')
set(gca,'xtick',1:5:67,'xticklabel',fxx(2:5:end))
colormap(redblue);
tp_editplots

% 
[h,p] = ttest(squeeze(nanmean(pxx_all,3)),zeros(size(squeeze(nanmean(pxx_all,3)))),'dim',3); h=p<fdr1(p(:),0.05,1);
subplot(3,3,2);
...
colormap(redblue); axis([1 31 1 62])
...
subplot(3,3,8);
[h,p] = ttest(rrr, zeros(size(rrr)),'dim',3); h = p<fdr1(p(:),0.05,1);
...
line([0 41],[0 0],'color','k','linestyle',':')
