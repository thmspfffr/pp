...
freqoi=2.^(1:(1/4):7);
%% Figure 5 (same as Figure 3, but for Task)
% -------------------------------------
...
tmp = squeeze(mean(plt_hh_cnt.corr_src,1))-squeeze(mean(plt_hh.corr_src,1));
...
par = mean(plt_hh_cnt.corr_src([V1_l V1_r],:,:),1)-mean(plt_hh.corr_src([V1_l V1_r],:,:),1);
...
par = mean(plt_hh_cnt.corr_src([A1_l A1_r],:,:),1)-mean(plt_hh.corr_src([A1_l A1_r],:,:),1);
[c,p]=permutest(squeeze(par),zeros(size(squeeze(par))),1,0.01,1000,2); h=[c{p<0.05}];
shadedErrorBar(log10(freqoi),nanmean(par,3),std(par,[],3)/sqrt(size(par,3)),'k')
if sum(h)>0
  plot(log10(freqoi(h)),nanmean(par(:,h,:),3),'k.','markersize',8)
...
par = mean(plt_hh_cnt.corr_src([M1_l M1_r],:,:),1)-mean(plt_hh.corr_src([M1_l M1_r],:,:),1);
[c,p]=permutest(squeeze(par),zeros(size(squeeze(par))),1,0.01,1000,2); h=[c{p<0.05}];
shadedErrorBar(log10(freqoi),nanmean(par,3),std(par,[],3)/sqrt(size(par,3)),'k')
if sum(h)>0
  plot(log10(freqoi(h)),nanmean(par(:,h,:),3),'k.','markersize',8)
...
par = mean(plt_hh_cnt.corr_src(find([dlpfc_l+dlpfc_r]),:,:),1)-mean(plt_hh.corr_src(find([dlpfc_l+dlpfc_r]),:,:),1);
[c,p]=permutest(squeeze(par),zeros(size(squeeze(par))),1,0.01,1000,2); h=[c{p<0.05}];
shadedErrorBar(log10(freqoi),nanmean(par,3),std(par,[],3)/sqrt(size(par,3)),'k')
if sum(h)>0
  plot(log10(freqoi(h)),nanmean(par(:,h,:),3),'k.','markersize',8)
...
pooled= plt_hh_cnt.corr_src_BNA(idx_sorted,:,:)-plt_hh.corr_src_BNA(idx_sorted,:,:);

[~,p]=ttest(pooled,zeros(size(pooled)),'dim',3); h=p<0.01;%h = p<fdr1(p(:),0.05,1); 
...
tmp = squeeze(mean(plt_hh_cnt.corr_src_df,1))-squeeze(mean(plt_hh.corr_src_df,1));
[c,p]=permutest(tmp,zeros(size(squeeze(tmp))),1,0.01,1000,2); h=[c{p<0.05}];
par = mean(tmp,2); 
par_std = std(tmp,[],2)/sqrt(size(tmp,2));
...
par = mean(plt_hh_cnt.corr_src_df([V1_l V1_r],:,:),1)-mean(plt_hh.corr_src_df([V1_l V1_r],:,:),1);
[c,p]=permutest(squeeze(par),zeros(size(squeeze(par))),1,0.01,1000,2); h=[c{p<0.05}];
shadedErrorBar(log10(freqoi),nanmean(par,3),std(par,[],3)/sqrt(size(par,3)),'k')
if sum(h)>0
  plot(log10(freqoi(h)),nanmean(par(:,h,:),3),'k.','markersize',8)
...
par = mean(plt_hh_cnt.corr_src_df([A1_l A1_r],:,:),1)-mean(plt_hh.corr_src_df([A1_l A1_r],:,:),1);
[c,p]=permutest(squeeze(par),zeros(size(squeeze(par))),1,0.01,1000,2); h=[c{p<0.05}];
shadedErrorBar(log10(freqoi),nanmean(par,3),std(par,[],3)/sqrt(size(par,3)),'k')
if sum(h)>0
  plot(log10(freqoi(h)),nanmean(par(:,h,:),3),'k.','markersize',8)
...
par = mean(plt_hh_cnt.corr_src_df([M1_l M1_r],:,:),1)-mean(plt_hh.corr_src_df([M1_l M1_r],:,:),1);
[c,p]=permutest(squeeze(par),zeros(size(squeeze(par))),1,0.01,1000,2); h=[c{p<0.05}];
shadedErrorBar(log10(freqoi),nanmean(par,3),std(par,[],3)/sqrt(size(par,3)),'k')
if sum(h)>0
  plot(log10(freqoi(h)),nanmean(par(:,h,:),3),'k.','markersize',8)
...
par = mean(plt_hh_cnt.corr_src_df(find([dlpfc_l+dlpfc_r]),:,:),1)-mean(plt_hh.corr_src_df(find([dlpfc_l+dlpfc_r]),:,:),1);
[c,p]=permutest(squeeze(par),zeros(size(squeeze(par))),1,0.01,1000,2); h=[c{p<0.05}];
shadedErrorBar(log10(freqoi),nanmean(par,3),std(par,[],3)/sqrt(size(par,3)),'k')
if sum(h)>0
  plot(log10(freqoi(h)),nanmean(par(:,h,:),3),'k.','markersize',8)
...
pooled= plt_hh_cnt.corr_src_df_BNA(idx_sorted,:,:)-plt_hh.corr_src_df_BNA(idx_sorted,:,:);
...
print(gcf,'-dpdf',sprintf('~/pp/plots/figure7.pdf'))
