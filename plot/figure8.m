...
for ifoi = [5 11 14 22]
  ...
  [h,p] = ttest(plt_hh_cnt.corr_src(:,ifoi,:)-plt_hh.corr_src(:,ifoi,:),zeros(size(plt_hh_cnt.corr_src(:,ifoi,:))),'dim',3);
  h=p<0.01;
  par=(nanmean(plt_hh_cnt.corr_src(:,ifoi,:),3)-nanmean(plt_hh.corr_src(:,ifoi,:),3)).*h;
  ...
  print(gcf,'-dpdf',sprintf('~/pp/plots/pp_figure8_dt0_f%d_v%d.tiff',ifoi,v))
...
for ifoi = [5 11 14 22]
  ...
  [h,p] = ttest(plt_hh_cnt.corr_src_df(:,ifoi,:)-plt_hh.corr_src_df(:,ifoi,:),zeros(size(plt_hh_cnt.corr_src_df(:,ifoi,:))),'dim',3);
  h=p<0.01;
  par=(nanmean(plt_hh_cnt.corr_src_df(:,ifoi,:),3)-nanmean(plt_hh.corr_src_df(:,ifoi,:),3)).*h;
  ...
  set(gcf,'renderer','painters')
  print(gcf,'-dpdf',sprintf('~/pp/plots/pp_figure8_dt1_f%d_v%d.tiff',ifoi,v))
