...
for isubj in SUBJLIST:
    for iblock in range(1,3):
        ...
        if v >33:
          ...
          np.save('/home/tpfeffer/pp/proc/src/pp_src_hh_fooof_result_pupilmeans_s%d_b%d_v%d.npy' % (isubj,iblock,v),'pup_means')
          ...
          fm.save('/home/tpfeffer/pp/proc/src/pp_hh_fooof_result_lo_s%d_b%d_v%d' % (isubj,iblock,v),save_results=True, save_settings=False,save_data=True);

          fm.fit(freqs, np.transpose(second_half), freq_range)
          fm.save('/home/tpfeffer/pp/proc/src/pp_hh_fooof_result_me_s%d_b%d_v%d' % (isubj,iblock,v),save_results=True, save_settings=False,save_data=True);

          fm.fit(freqs, np.transpose(third_half), freq_range)
          fm.save('/home/tpfeffer/pp/proc/src/pp_hh_fooof_result_hi_s%d_b%d_v%d' % (isubj,iblock,v),save_results=True, save_settings=False,save_data=True);
          ...
          scipy.io.savemat('/home/tpfeffer/pp/proc/src/pp_hh_collected_fooof_s%d_b%d_v%d.mat' % (isubj,iblock,v), {'g': g, 'gg': gg,  'aper':  aper, 'pup': pup, 'pup_df': pup_df})
