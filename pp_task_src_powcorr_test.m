...
for isubj = 33
  for m = 3
%     
%     if ~exist(sprintf([outdir 'pp_cnt_src_powcorr_test_s%d_m%d_v%d_processing.txt'],isubj,m,v))
%       system(['touch ' outdir sprintf('pp_cnt_src_powcorr_test_s%d_m%d_v%d_processing.txt',isubj,m,v)]);
%     else
%       continue
%     end
...
%         cs = tp_wavelet_crossspec(data,foi_range(ifoi),width);
        cs = tp_compute_csd_wavelets(data,para);
...
%         end
    ...
    save(sprintf([outdir 'pp_cnt_src_powcorr_test_s%d_m%d_v%d.mat'],isubj,m,v),'powcorr');
