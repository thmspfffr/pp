...
for isubj in range(1,25):
    for iblock in range(1,2):
        ...
        if os.path.exists('/home/tpfeffer/pp/proc/src/pp_gla_fooof_s%d_b%d_v%d_proc.txt' % (isubj,iblock,v)) == True:
            continue

        os.system('touch /home/tpfeffer/pp/proc/src/pp_gla_fooof_s%d_b%d_v%d_proc.txt' % (isubj,iblock,v))
        try:
        ...
        if v < 3: 
          ...
          scipy.io.savemat('/home/tpfeffer/pp/proc/src/pp_gla_collected_fooof_s%d_b%d_v%d.mat' % (isubj,iblock,v), {'g': g,  'aper':  aper})
