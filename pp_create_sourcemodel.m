...
for im = 1:3
  for isubj = SUBJLIST
    ...
    %
%     if ~exist(sprintf([outdir 'pp_create_sourcemodel_s%d_m%d_v%d_processing.txt'],isubj,im,v))
%       system(['touch ' outdir sprintf('pp_create_sourcemodel_s%d_m%d_v%d_processing.txt',isubj,im,v)]);
%     else
%       continue
%     end
    % %
...
%         sa_template.grid_cortex_brainstorm = cortex_brainstorm.pos;
      ...
      if      strcmp(grid_size,'BNA')
        ...
        save([outdir sprintf('pp_sa_s%d_m%d_b%d_v%d.mat',isubj,im,iblock,v)],'sa');
        close all
      end
      br = 0;
    end
